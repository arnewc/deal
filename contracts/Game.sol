...
contract Game is GameHelpers
{
    ...
    function commit_bob(
        uint _index,
        uint8 alice_index
    ) validIndex(_index) atState(_index, State.Commit_Bob) transitionNext(_index) public returns (uint[52] memory)
    {
    ...
    function play_bob(
        uint _index,
        uint card
    ) validIndex(_index) onlyBob(_index) atState(_index, State.Play_Bob) transitionNext(_index) public
    {
    ...
    function play_alice(
        uint _index,
        uint _bob_card
    ) validIndex(_index) onlyAlice(_index) atState(_index, State.Play_Alice) transitionNext(_index) public
    {
        emit Play(msg.sender, _index, State.Play_Alice);
        data[_index].bob_card = _bob_card;
    }

    function reveal_alice(
        uint _index,
        uint alice_secret
    ) validIndex(_index) onlyAlice(_index) atState(_index, State.Reveal_Alice) transitionNext(_index) public
    {
    ...
    function reveal_bob(
        uint _index,
        uint bob_secret
    ) validIndex(_index) onlyBob(_index) atState(_index, State.Reveal_Bob) transitionNext(_index) public
    {
        emit Play(msg.sender, _index, State.Reveal_Bob);
        data[_index].bob_secret = bob_secret;
    }

    function verify(uint _index) validIndex(_index) atState(_index, State.Verify) public returns (bool, address)
    {
        Instance memory instance = data[_index];
        uint alice_card_decrypted = Math.expmod(instance.cards[instance.alice_index], instance.alice_secret, N);
        uint bob_card_decrypted = Math.expmod(instance.bob_card, instance.bob_secret, N);
        ...
        return (true, isAlice ? data[_index].alice : data[_index].bob);
    }
}
