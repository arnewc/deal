...
abstract contract GameHelpers is GameData
{
    ...
    {
        require (data[_index].state == _state,"Function cannot be called at this state.");
        _;
    }

    modifier onlyAlice(uint _index)
    {
      require(msg.sender == data[_index].alice, "Sender is not Alice.");
      _;
    }

    modifier onlyBob(uint _index)
    {
      require(msg.sender == data[_index].bob, "Sender is not Bob.");
      _;
    }
